...
class SaverBot(TelegramBot):
    ...
    async def register_user(self, update, context):        
        try:
            access_token, refresh_token = self.get_oauth_tokens(update, context)
    ...
    def get_oauth_tokens(self, update, context):
    ...
    async def upload_file(self, update: Update, context: CallbackContext) -> None:
        ...
        if 'access_token' in context.chat_data:
            ...
            try:
                ...
                uploaded_file = await self._upload_file(update, folder_id, box_client)
            ...
            await update.message.reply_text('It seems like you have to connect to Box.com.')
            await self.send_register_link(update, context)

    async def _upload_file(self, update, folder_id, box_client):
        document = update.effective_message.document
        file = await self.application.bot.get_file(document.file_id)
        with io.BytesIO() as content:
            await file.download(out=content)
            uploaded_file: BoxFile = box_client.folder(folder_id).upload_stream(content, document.file_name)
